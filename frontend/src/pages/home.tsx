...
const DEPARTMENT_NEWS = [
  {
    id: 1,
    title: "Ra mắt gói cước 5G MobiFone Ultra tốc độ cao",
    department: "Kinh doanh",
    deptColor: "bg-emerald-100 text-emerald-700",
    summary: "MobiFone chính thức triển khai gói cước 5G tốc độ cao mới nhất, phủ sóng 63 tỉnh thành trên toàn quốc từ tháng 5/2026.",
    date: "22/04/2026",
    gradient: "from-emerald-400 to-teal-500",
  },
  {
    id: 2,
    title: "Nâng cấp hệ thống core network Q2/2026",
    department: "IT",
    deptColor: "bg-blue-100 text-blue-700",
    summary: "Phòng IT hoàn thành lộ trình nâng cấp hạ tầng mạng lõi, đảm bảo uptime 99.99% phục vụ khách hàng toàn quốc.",
    date: "20/04/2026",
    gradient: "from-blue-400 to-indigo-500",
  },
  {
    id: 3,
    title: "Chiến dịch tuyển dụng nhân tài công nghệ 2026",
    department: "HR",
    deptColor: "bg-purple-100 text-purple-700",
    summary: "HR mở 50 vị trí tuyển dụng cho các kỹ sư phần mềm, chuyên gia AI và data analyst trong Q2/2026.",
    date: "18/04/2026",
    gradient: "from-purple-400 to-violet-500",
  },
  {
    id: 4,
    title: "Kết quả khảo sát hài lòng khách hàng Q1",
    department: "Marketing",
    deptColor: "bg-orange-100 text-orange-700",
    summary: "Chỉ số NPS đạt 78 điểm, tăng 12 điểm so với cùng kỳ năm ngoái — kết quả tốt nhất trong 5 năm gần đây.",
    date: "15/04/2026",
...
const UPCOMING_EVENTS = [
  {
    id: 1,
    title: "Họp toàn công ty Q2",
    date: "2026-04-25",
    time: "09:00",
    location: "Hội trường A, Tầng 10",
    type: "meeting",
    typeLabel: "Họp",
    color: "from-[#1F4E79] to-[#2E75B6]",
    tagColor: "bg-blue-100 text-blue-700",
  },
  {
    id: 2,
    title: "Deadline nộp báo cáo Q1",
    date: "2026-04-30",
    time: "17:00",
    location: "Cổng nội bộ",
    type: "deadline",
    typeLabel: "Deadline",
  ...
  {
    id: 3,
    title: "Khóa đào tạo an ninh mạng",
    date: "2026-05-05",
    time: "08:30",
    location: "Phòng Training B, Tầng 6",
    type: "training",
    typeLabel: "Đào tạo",
  ...
  {
    id: 4,
    title: "Team Building IT Department",
    date: "2026-05-10",
    time: "07:00",
...
export default function Home() {
  ...
  return (
    <div className={`flex flex-col h-full overflow-y-scroll custom-scrollbar ${bg} transition-colors duration-300`}>
      {/* ── Hero ──────────────────────────────────────────────────────────────── */}
      ...
      <div className="flex-1 p-6 space-y-6">
        ...
        {/* Row 1: Activity Feed + Right sidebar */}
        <div className="grid grid-cols-3 gap-6">

          {/* Activity Feed */}
          <div className={`col-span-2 rounded-2xl shadow-sm border ${card} overflow-hidden`}>
            ...
            <div className={`divide-y ${divider}`}>
              ...
              {!isLoadingPosts && posts.length === 0 && (
                <div className={`py-10 text-center text-sm ${textSub}`}>Chưa có bài đăng nào</div>
              )}
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {post.authorName && (
                        <div className="flex items-center gap-1.5 ml-auto">
                          <div className="w-5 h-5 rounded-full bg-[#1F4E79] flex items-center justify-center">
                            <span className="text-white text-[9px] font-bold">{post.authorName.charAt(0)}</span>
                          </div>
                          <span className={`text-xs ${textSub}`}>{post.authorName}</span>
                        </div>
                      )}
                    </div>
                  </div>
          ...
          {/* Right: Announcements + Online Members */}
            ...
            <div className={`rounded-2xl shadow-sm border ${card} overflow-hidden`}>
              <div className={`px-5 py-4 border-b flex items-center justify-between ${divider}`}>
                ...
                <button onClick={() => navigate("/chat")}
                  className="text-xs text-[#1F4E79] hover:underline font-medium flex items-center gap-1">
        ...
        {/* Row 2: Department News + Upcoming Events */}
        <div className="grid grid-cols-3 gap-6">
            ...
            <div className="grid grid-cols-2 gap-0">
              {DEPARTMENT_NEWS.map((news, index) => (
                <div key={news.id}
                  ...
                  <h3 className={`font-semibold text-sm leading-snug mb-1.5 line-clamp-2 ${textMain}`}>
                    {news.title}
                  </h3>
                  <p className={`text-xs leading-relaxed line-clamp-2 mb-2 ${textSub}`}>
                    {news.summary}
                  </p>
                ...
                return (
                  <div key={event.id}
                    className={`rounded-xl p-3 cursor-pointer transition-colors border ${
                      darkMode ? "border-[#2a3a5c] hover:bg-[#1f2e4a]" : "border-gray-100 hover:bg-gray-50"
                    }`}>
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between gap-1 mb-1">
                          ...
                          {isUrgent && (
                            <span className="text-[9px] bg-red-100 text-red-600 font-bold px-1.5 py-0.5 rounded-full flex-shrink-0">
                              {daysUntil}
                            </span>
                          )}
                        </div>
                        ...
                        {!isUrgent && (
                          <span className={`inline-block mt-1.5 text-[10px] font-medium px-2 py-0.5 rounded-full ${event.tagColor}`}>
                            {daysUntil}
                          </span>
                        )}
                      </div>
