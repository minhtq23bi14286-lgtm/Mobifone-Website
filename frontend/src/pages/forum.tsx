...
export default function Forum() {
  ...
  const [activeTag, setActiveTag] = useState<string | null>(null); // ✅ trong component
  ...
  const previewAttachments: Attachment[] = selectedPost?.attachments?.map(a => {
    try { return JSON.parse(a); } catch { return null; }
  }).filter(Boolean) || [];
  const previewColorIndex = selectedPost
    ? filteredPosts.findIndex(p => p.id === selectedPost.id) % POST_COLORS.length
    : 0;
  ...
  return (
    <div className={`h-full flex flex-col ${bg}`}>
      <div className="flex-1 overflow-hidden flex">
        ...
        {/* ── Col 1: Sidebar ── */}
        <div className={`w-72 flex-shrink-0 border-r overflow-y-auto ${panelBg} ${border}`}>
            ...
            <div>
              ...
              <div className="space-y-0.5">
                ...
                className={`w-full text-left px-3 py-2.5 rounded-xl text-sm font-medium transition-colors flex items-center gap-2 ${
                      activeCategory === cat ? "bg-[#1F4E79] text-white" : `${textSub} ${hoverBg}`
                    }`}>
              ...
              <div className="flex items-center gap-1">
                {SORT_OPTIONS.map(opt => (
                  <button key={opt.value} onClick={() => setSortBy(opt.value)}
                    ...
                    } ${sortBy === opt.value ? "bg-[#1F4E79] text-white" : `${textSub} ${hoverBg}`}`}>
                ...
                <div key={post.id} onClick={() => setSelectedPost(isSelected ? null : post)}
                  ...
                  <div className={compact ? "p-4" : "p-5"}>
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <div className="flex items-center gap-1">
                        <span className={`px-2 py-0.5 rounded-full font-medium ${compact ? "text-[10px]" : "text-xs"} ${darkMode ? "bg-blue-500/10 text-blue-400" : "bg-blue-50 text-[#1F4E79]"}`}>
                          {post.category}
                        </span>
                    ...
                    <h3 className={`font-bold mb-2 leading-snug hover:text-[#1F4E79] transition-colors ${textMain} ${compact ? "text-sm" : "text-xl"}`}>
                      {post.title}
                    </h3>
            ...
            <div className="p-4 space-y-4">
              ...
              <div className={`flex gap-4 py-3 px-4 rounded-xl ${darkMode ? "bg-white/5" : "bg-gray-50"}`}>
                {[
                  ...
                  { icon: MessageCircle, value: selectedPost.comments, label: "bình luận" },
